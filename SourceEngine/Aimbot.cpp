    ...
    if (&MenuConfig::doRagebot && SDK::LocalPawn != nullptr) {
        if (&MenuConfig::rageSilent) {
            ...
            if (ClosestEntity != nullptr)
            {
                ...
                if (Entity.CanHit) {
                    pCmd->SetSubTickAngle(TargetAngles);
                    //do triggerbot after setting viewangles
            ...
            //normal ragebot logic here
            //this shit boutta lookm like texashook
            Vector2D_t MousePos = { ImGui::GetMousePos().x,  ImGui::GetMousePos().y };
            ...
            if (MinDistanceToMouse <= MenuConfig::FOV)
            {
                Vector_t PlayerViewPosition = SDK::LocalPawn->GetOrigin() + SDK::LocalPawn->GetViewOffset();
                QAngle_t TargetAngles = CalculateAngleScalar(PlayerViewPosition, ClosestBoneWorldPos);
                ...
                if (Entity.CanHit) { //only do comical gaming if we can hit them ofc
                    TargetAngles.Normalize();
                    TargetAngles.Clamp();
